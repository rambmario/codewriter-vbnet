...
Public Class frmPrincipal
    ...
    ' Defino el array donde se guardar�n los datos de la tabla
    'Dim arrEstructura() As regSistemaTabla

    Const baseCon As String = "Integrated Security=SSPI;Persist Security Info=False;Initial Catalog={1};Data Source={0};Type System Version=SQL Server 2012;"
    ...
    Private Sub frmPrincipal_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Me.cmbServer.Text = ".\SQLExpress16"
        Try
    ...
    Private Sub chkNombresAutomaticos_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkNombresAutomaticos.CheckedChanged
        ' Verirfico el estado del campo nombres autom�ticos
    ...
    Private Sub chkTodasLasTablas_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkTodasLasTablas.CheckedChanged
        ' Verifico si est� o no marcado el control para crear todas las clases de una db
    ...
    Private Sub btnGenerarClase_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGenerarClase.Click
        ...
        If Not chkTodasLasTablas.Checked Then
            ' Solicito confirmaci�n antes de crear la clase
            If MessageBox.Show("Desea Crear la Clase '" & txtClase.Text & "' Para la Tabla '" & txtTabla.Text & "' ?", "CONFIRMACION", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                ...
                MessageBox.Show("Se Gener� la Clase '" & txtClase.Text & "' en el Archivo '" & txtArchivo.Text & "' de la Tabla '" & txtTabla.Text & "'." & Chr(13) & Chr(13) & "El Archivo se Guard� en C:\." & Chr(13) & Chr(13) & "Copiar la Clase en el Proyecto y Agregarla con --> Agregar elemento existente", "GENERACION DE CLASES", MessageBoxButtons.OK, MessageBoxIcon.Information)
        ...
        Else
            ...
            da.Fill(dt)
            ' Solicito confirmaci�n antes de crear todas las tablas
            If MessageBox.Show("Desea las Clases Para Todas las Tablas de la Base de Datos Seleccionada ?", "CONFIRMACION", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                ...
                For Each row As DataRow In dt.Rows
                    ...
                    indice = indice + 1
                Next
                MessageBox.Show("Se Gener� la Clase Para Todas las Tablas de la Base de Datos." & Chr(13) & Chr(13) & "Los Archivos se Guardaron en C:\." & Chr(13) & Chr(13) & "Copiar la Clase en el Proyecto y Agregarla con --> Agregar elemento existente", "GENERACION DE CLASES", MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Private Sub CambioNombreTabla(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtTabla.KeyUp
        ' Verifico si se est� en nombres autom�ticos
        If chkNombresAutomaticos.Checked Then
            Call GenerarNombres()
        End If
    End Sub
    ...
    ' Este procedimiento recibe una cadena conexi�n y el nombre de una tabla y carga un array con el nombre y tiposql de cada uno de los campos de la tabla.
    ...
    Private Sub Cargar(ByVal conexion As String, ByVal tabla As String)
        ...
        Dim daSchema As New SqlDataAdapter("SELECT SCHEMA_NAME(schema_id) as nombre FROM sys.tables WHERE sys.tables.name = '" & tabla & "'", cn)
        Dim dtSchema As New DataTable

        daSchema.Fill(dtSchema)

        Dim schema As String = dtSchema.Rows(0).Item("nombre").ToString


        Dim da As New SqlDataAdapter("SELECT * FROM [" & schema & "].[" & tabla & "]", cn) ' los [] ban por si es una tabla de nombre compuesto
        ...
        'Dim daSistema As New SqlDataAdapter("SELECT sys.tables.name AS tabla ,sys.columns.[scale] ,sys.columns.[name] AS campo ,sys.types.name AS tipo ,sys.columns.is_nullable  AS [isnull] ,[column_id] AS colorder ,sys.columns.[max_length] AS longitud ,sys.columns.[precisiON] AS prec " &
        '               "FROM sys.tables INNER JOIN sys.columns ON sys.columns.object_id=sys.tables.object_id INNER JOIN	sys.types ON sys.types.system_type_id=sys.columns.system_type_id  " &
        '               "WHERE sys.tables.name = '" & tabla & "' and sys.types.name not in('hierarchyid','geometry','sysname') ORDER BY sys.columns.column_id", cn)

        Dim daSistema As New SqlDataAdapter("SELECT '['+SCHEMA_NAME(sys.tables.schema_id)+'].['+ sys.tables.name+']' AS tabla ,sys.columns.[scale] ,sys.columns.[name] AS campo ,sys.types.name AS tipo ,sys.columns.is_nullable  AS [isnull] ,[column_id] AS colorder ,sys.columns.[max_length] AS longitud ,sys.columns.[precisiON] AS prec " &
                       "FROM sys.tables INNER JOIN sys.columns ON sys.columns.object_id=sys.tables.object_id INNER JOIN	sys.types ON sys.types.system_type_id=sys.columns.system_type_id  " &
                       "WHERE sys.tables.name = '" & tabla & "' and sys.types.name not in('hierarchyid','geometry','sysname','AccountNumber','Name','OrderNumber','Phone') ORDER BY sys.columns.column_id", cn)
        ...
        For Each dc In dt.Columns()
            ...
            ' La f�rmula en el tiposql es para eliminar el "system." si llegase a aparecer
            arrEstructura(indice).tiposql = Mid$(dc.DataType.ToString, InStr(dc.DataType.ToString, ".", CompareMethod.Text) + 1)
            ' Incremento la posisi�n dentro del array
    ...
    ' Este procedimiento genera un Archivo de texto con extensi�n .VB con la definici�n de una clase correspondiente a una tabla de una base de datos.
    ' Al basarse enteramente en la estructura de una tabla de SQL Server, las propiedades de la clases son el mismo tiposql que en la tabla "f�sica", por
    ...
    ' Esto no genera ning�n problema, ya que VB.NET convierte en forma autom�tica los datos (ya que las clase toma los datos "oficiales" de .NET).
    ...
    '            ' Creo una l�nea divisoria (espacio en blanco)
    '            '' PrintLine(FileCls, "")
    ...
    '    PrintLine(FileCls, TAB(5), "'importa los datos de una campa�a desde el txt")
    ...
    '        PrintLine(FileCls, TAB(4), "'''la informaci�n y funcionalidad para la ejecuci�n de pruebas actual. ")
    '        PrintLine(FileCls, TAB(4), "'''</summary>")
    ...
    '        PrintLine(FileCls, TAB(4), "'")
    '        PrintLine(FileCls, TAB(4), "'Use ClassInitialize para ejecutar c�digo antes de ejecutar la primera prueba en la clase ")
    ...
    '        PrintLine(FileCls, TAB(4), "'")
    '        PrintLine(FileCls, TAB(4), "'Use ClassCleanup para ejecutar c�digo despu�s de haber ejecutado todas las pruebas en una clase")
    '        PrintLine(FileCls, TAB(4), "'<ClassCleanup()>  _")
    '        PrintLine(FileCls, TAB(4), "'Public Shared Sub MyClassCleanup()")
    '        PrintLine(FileCls, TAB(4), "'End Sub")
    '        PrintLine(FileCls, TAB(4), "'")
    '        PrintLine(FileCls, TAB(4), "'Use TestInitialize para ejecutar c�digo antes de ejecutar cada prueba")
    '        PrintLine(FileCls, TAB(4), "'<TestInitialize()>  _")
    '        PrintLine(FileCls, TAB(4), "'Public Sub MyTestInitialize()")
    '        PrintLine(FileCls, TAB(4), "'End Sub")
    '        PrintLine(FileCls, TAB(4), "'")
    '        PrintLine(FileCls, TAB(4), "'Use TestCleanup para ejecutar c�digo despu�s de que se hayan ejecutado todas las pruebas")
    ...
    '    PrintLine(FileFrm, TAB(4), "End Sub")
    '    PrintLine(FileFrm, "")
    '    PrintLine(FileFrm, TAB(4), "'Requerido por el Dise�ador de Windows Forms")
    '    PrintLine(FileFrm, TAB(4), "Private components As System.ComponentModel.IContainer")
    '    PrintLine(FileFrm, "")
    '    PrintLine(FileFrm, TAB(4), "'NOTA: el Dise�ador de Windows Forms necesita el siguiente procedimiento")
    '    PrintLine(FileFrm, TAB(4), "'Se puede modificar usando el Dise�ador de Windows Forms.")
    '    PrintLine(FileFrm, TAB(4), "'No lo modifique con el editor de c�digo.")
    ...
    '        PrintLine(FileFrmDetalle, TAB(4), "End Sub")
    '        PrintLine(FileFrmDetalle, "")
    '        PrintLine(FileFrmDetalle, TAB(4), "'Requerido por el Dise�ador de Windows Forms")
    '        PrintLine(FileFrmDetalle, TAB(4), "Private components As System.ComponentModel.IContainer")
    '        PrintLine(FileFrmDetalle, "")
    '        PrintLine(FileFrmDetalle, TAB(4), "'NOTA: el Dise�ador de Windows Forms necesita el siguiente procedimiento")
    '        PrintLine(FileFrmDetalle, TAB(4), "'Se puede modificar usando el Dise�ador de Windows Forms.")
    '        PrintLine(FileFrmDetalle, TAB(4), "'No lo modifique con el editor de c�digo.")
    ...
    Sub ParseStreamAndActionScript(ByVal stream As System.IO.StreamReader)
        ...
        While line IsNot Nothing
            ...
            ElseIf line = "/*FINDELSCRIPT*/" Then

                ' Add GO at end of the file
                currentBlock.Append(";")
                currentBlock.Append(Environment.NewLine)
                line = stream.ReadLine()

                Debug.WriteLine("total de lineas: " & blockLine)
    ...
    Sub ActionScript(ByVal script As String)
        ...
        Catch ex As SqlException
            BanderaSP = 1
            MessageBox.Show("Error N�: " & ex.Number & " En el SP: " & ex.Procedure & " Linea N�: " & ex.LineNumber, "MENSAJE", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Exit Try
        End Try
    End Sub
    ...
    Private Sub btncop_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSP.Click
        ...
        Else
            MessageBox.Show("Se ha Producido un Error en la Ejecuci�n ", "MENSAJE", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        End If
    End Sub
    ...
    Private Sub lstBaseDato_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lstBaseDato.SelectedIndexChanged
        ...
        'Me.txtPath.Text = "F:\VBNet\" & lstBaseDato.SelectedValue.ToString() & "\trunk\cop\"
        Me.txtPath.Text = "F:\VBNet\script\"
    ...
    Private Sub btnTabla_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnTabla.Click
        Dim cn As New SqlConnection(Me.txtCadenaConexion.Text)
        Dim da As New SqlDataAdapter("SELECT TOP 100 PERCENT name, id FROM dbo.sysobjects WHERE (type = 'U') AND name <> 'dtproperties' ORDER BY name", cn)
        'Dim da As New SqlDataAdapter("SELECT SCHEMA_NAME(schema_id)+'.'+name,  object_id as id FROM sys.tables ORDER BY name", cn)
    ...
    Private Sub btnTruncate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnTruncate.Click
        ...
        If Not chkTodasLasTablas.Checked Then
            ...
            If MessageBox.Show("Desea Crear el Truncate '" & txtClase.Text & "' Para la Tabla '" & txtTabla.Text & "' ?", "CONFIRMACION", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                ...
                ' Indico que se ha generado la clase
                MessageBox.Show("Se Gener� el Truncate '" & txtClase.Text & "' en el Archivo '" & "Truncate.sql" & "' de la Tabla '" & txtTabla.Text & "'." & Chr(13) & Chr(13) & "El Archivo se Guard� en C:\." & Chr(13) & Chr(13) & "Ejecutar el Script", "GENERACION DE SCRIPT", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Me.btnSP.Enabled = True
            End If
        Else
            Dim cn As New SqlConnection(Me.txtCadenaConexion.Text)
            ...
            If MessageBox.Show("Desea Crear los Truncates Para Todas las Tablas de la Base de Datos Seleccionada ?", "CONFIRMACION", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                ...
                For Each row As DataRow In dt.Rows
                    ...
                    indice = indice + 1
                Next
                MessageBox.Show("Se Gener� el Truncate Para Todas las Tablas de la Base de Datos." & Chr(13) & Chr(13) & "Los Archivos se Guardaron en C:\." & Chr(13) & Chr(13) & "Ejecutar el Script", "GENERACION DE SCRIPT", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Me.btnTodoSp.Enabled = True
            End If
        End If
        FileClose(FileInsert2)
    ...
    Private Sub btnIdSuplente_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnIdSuplente.Click
        ...
        If Not chkTodasLasTablas.Checked Then
            ...
            If MessageBox.Show("Desea Crear la Clave Suplente '" & txtClase.Text & "' Para la Tabla '" & txtTabla.Text & "' ?", "CONFIRMACION", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                ...
                ' Indico que se ha generado la clase
                MessageBox.Show("Se Gener� la Clave Suplente '" & txtClase.Text & "' en el Archivo '" & "Clave_suplente.sql" & "' de la Tabla '" & txtTabla.Text & "'." & Chr(13) & Chr(13) & "El Archivo se Guard� en C:\." & Chr(13) & Chr(13) & "Ejecutar con SQL", "GENERACION DE SCRIPTS", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Me.btnSP.Enabled = True
            End If
        Else
            Dim cn As New SqlConnection(Me.txtCadenaConexion.Text)
            ...
            If MessageBox.Show("Desea Crear las Claves Suplentes Para Todas las Tablas de la Base de Datos Seleccionada ?", "CONFIRMACION", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                ...
                For Each row As DataRow In dt.Rows
                    ...
                    indice = indice + 1
                Next
                MessageBox.Show("Se Gener� El Script Para Todas las Tablas de la Base de Datos." & Chr(13) & Chr(13) & "Los Archivos se Guardaron en C:\." & Chr(13) & Chr(13) & "Copiar la Clase en el Proyecto y Agregarla con --> Agregar elemento existente", "GENERACION DE CLASES", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Me.btnTodoSp.Enabled = True
            End If
        End If
        FileClose(FileInsert2)
    ...
    Private Sub btnExist_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExist.Click
        ...
        If Not chkTodasLasTablas.Checked Then
            ...
            If MessageBox.Show("Desea Crear el Exist '" & txtClase.Text & "' Para la Tabla '" & txtTabla.Text & "' ?", "CONFIRMACION", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                ...
                ' Indico que se ha generado la clase
                MessageBox.Show("Se Gener� el Exist '" & txtClase.Text & "' en el Archivo '" & "Exist.sql" & "' de la Tabla '" & txtTabla.Text & "'." & Chr(13) & Chr(13) & "El Archivo se Guard� en C:\." & Chr(13) & Chr(13) & "Ejecutar el Script", "GENERACION DE SCRIPT", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Me.btnSP.Enabled = True
            End If
        Else
            Dim cn As New SqlConnection(Me.txtCadenaConexion.Text)
            ...
            If MessageBox.Show("Desea Crear los Exist Para Todas las Tablas de la Base de Datos Seleccionada ?", "CONFIRMACION", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                ...
                For Each row As DataRow In dt.Rows
                    ...
                    indice = indice + 1
                Next
                MessageBox.Show("Se Gener� el Exist Para Todas las Tablas de la Base de Datos." & Chr(13) & Chr(13) & "Los Archivos se Guardaron en C:\." & Chr(13) & Chr(13) & "Ejecutar el Script", "GENERACION DE SCRIPT", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Me.btnTodoSp.Enabled = True
            End If
        End If
        FileClose(FileInsert2)
    ...
    Private Sub btnSincro_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSincro.Click
        ...
        If Not chkTodasLasTablas.Checked Then
            ...
            If MessageBox.Show("Desea Crear el Sincro '" & txtClase.Text & "' Para la Tabla '" & txtTabla.Text & "' ?", "CONFIRMACION", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                ...
                ' Indico que se ha generado la clase
                MessageBox.Show("Se Gener� el Sincro '" & txtClase.Text & "' en el Archivo '" & "Truncate.sql" & "' de la Tabla '" & txtTabla.Text & "'." & Chr(13) & Chr(13) & "El Archivo se Guard� en C:\." & Chr(13) & Chr(13) & "Ejecutar el Script", "GENERACION DE SCRIPT", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Me.btnSP.Enabled = True
            End If
        Else
            Dim cn As New SqlConnection(Me.txtCadenaConexion.Text)
            ...
            If MessageBox.Show("Desea Crear los Sincro Para Todas las Tablas de la Base de Datos Seleccionada ?", "CONFIRMACION", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                ...
                For Each row As DataRow In dt.Rows
                    ...
                    indice = indice + 1
                Next
                MessageBox.Show("Se Gener� el Sincro Para Todas las Tablas de la Base de Datos." & Chr(13) & Chr(13) & "Los Archivos se Guardaron en C:\." & Chr(13) & Chr(13) & "Ejecutar el Script", "GENERACION DE SCRIPT", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Me.btnTodoSp.Enabled = True
            End If
        End If
        FileClose(FileInsert2)
    ...
    Private Sub btnClase2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Not chkTodasLasTablas.Checked Then
            ...
            If MessageBox.Show("Desea Crear la Clase '" & txtClase.Text & "' Para la Tabla '" & txtTabla.Text & "' ?", "CONFIRMACION", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                ...
                oTest.Iniciar()
                oTest.ImprimirTest(Me.lstBaseDato.Text, Me.txtPath.Text, Me.txtTabla.Text,
                Me.chkTestInsert.Checked, Me.chkTestDelete.Checked, Me.chkTestGetall.Checked, Me.chkTestGetone.Checked,
                Me.chkTestGetcmb.Checked, Me.chkTestUpdate.Checked, Me.chkTestExist.Checked, Me.chkTestFind.Checked)

                oSql.Iniciar()
                oSql.ImprimirSps(Me.lstBaseDato.Text, Me.txtPath.Text, Me.txtTabla.Text, NumeroFilas,
                Me.chkInsert.Checked, Me.chkDelete.Checked, Me.chkGetAll.Checked, Me.chkGetOne.Checked, Me.chkGetCmb.Checked,
                Me.chkUpdate.Checked, Me.chkExist.Checked, Me.chkFind.Checked, Me.chkInsertOne.Checked,
                Me.chkUpdateID.Checked, Me.chkDeleteEncabezado.Checked, Me.chkGetAllEncabezadoOne.Checked)
    ...
    Private Sub chk2008_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chk2008.CheckedChanged
        If Me.chk2008.Checked Then
            Me.cmbServer.Text = ".\SQLExpress19"
            Me.btnBaseDato.Enabled = True
        Else
            Me.cmbServer.Text = ""
            Me.btnBaseDato.Enabled = False
    ...
    Private Sub btnAbrir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAbrir.Click
        Try
            With ofdScript
                ' Vea btnRetriveFileNames_Click para obtener una explicaci�n de los valores predeterminados
                ' de las propiedades.

                ' Comprobar que el archivo seleccionado existe. El cuadro de di�logo muestra
                ' una advertencia en caso contrario.
                .CheckFileExists = True

                ' Comprobar que la ruta de acceso seleccionada existe. El cuadro de di�logo muestra 
                ' una advertencia en caso contrario.
                .CheckPathExists = True

                ' Obtener o establecer una extensi�n predeterminada. No incluye el "." inicial.
                .DefaultExt = "sql"

                ' �Devolver el archivo al que hace referencia un v�nculo? Si False, devuelve el archivo de v�nculo
                ...
                ' par consta de una especificaci�n descripci�n|archivo. Utilice "|" entre los pares. No es necesario
                ' poner "|" al final. Puede establecer la propiedad FilterIndex tambi�n, para seleccionar el
                ' filtro predeterminado. El primer filtro tiene el n�mero 1 (no 0). El valor predeterminado es 1. 
                .Filter = _
                "Archivos SQL (*.sql)|*.sql" '|All files|*.*"

                ' .Multiselect = False

                ' �Restaurar el directorio original despu�s de seleccionar
                ' un archivo? Si False, el directorio actual cambia
                ' al directorio en el que seleccion� el archivo.
                ' Establ�zcalo como True para poner la carpeta actual de nuevo
                ' donde estaba cuando comenz�.
                ' El valor predeterminado es False.
                .RestoreDirectory = True

                ' �Mostrar el bot�n Ayuda y la casilla de verificaci�n S�lo lectura?
                .ShowHelp = True
                .ShowReadOnly = False

                ' �Comenzar con la casilla de verificaci�n S�lo lectura activada?
                ' Esto s�lo tiene sentido si ShowReadOnly es True.
                .ReadOnlyChecked = False

                .Title = "Select a file to open"

                ' �Aceptar s�lo nombres de archivo Win32 v�lidos?
                .ValidateNames = True
    ...
    Private Sub btnRunScript_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRunScript.Click
        ...
        ' Si files no tiene archivos muestro un mensaje
        If files.Length = 0 Then
            MsgBox("No hay archivos seleccionados", MsgBoxStyle.Exclamation, Me.Text)
            Exit Sub
        End If
